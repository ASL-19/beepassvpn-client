...
import autoLaunch from 'auto-launch'; // tslint:disable-line
...
function setupWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    // TODO: ONLY for debug on bigger view, will change back to 360 and 640
    width: 1000,
    height: 800,
    resizable: false,
    webPreferences: {
      // ASL19: Why doesn’t this work in debug mode?
      devTools: debugMode,
  ...
  // ASL19: set path to beepass-ui react index.html
  const pathToIndexHtml = path.join(app.getAppPath(), 'www', 'index.html');
...
function main() {
  ...
  ipcMain.on('outline-ipc-openLandingPage', (event: Event, url: string) => {
    shell.openExternal(url);
    event.preventDefault()
  });
