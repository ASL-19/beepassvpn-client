...
export class ElectronRendererMethodChannel {
  ...
  public readonly invoke = (channel: string, ...args: unknown[]): Promise<any> => {
    console.info(`invoke ${channel} with ${JSON.stringify(args, null, 2)}`)
    return ipcRenderer.invoke(`${this.namespace}-${channel}`, ...args);
  }
  ...
  public readonly removeAllListener = (channel: string): void => {
    ipcRenderer.removeAllListeners(`${this.namespace}-${channel}`);
  };
}
